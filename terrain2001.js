// Load Landsat 7 imagery for 2001
var image = ee.ImageCollection("LANDSAT/LE07/C02/T1_L2")
    ...
    .median()
    .clip(mysore);
print(image, 'Landsat Image 2001');
Map.centerObject(mysore, 12);

// Visualization parameters for true color image
var visParamsTrue = {
    bands: ['SR_B3', 'SR_B2', 'SR_B1'], // Landsat 7 bands: Red, Green, Blue
    gamma: [1.5, 1.5, 1.0]
};
Map.addLayer(image, visParamsTrue, 'Landsat 2001');

// Ensure that the 'class' property is set as a numeric type for each collection
var water = water.map(function(feature) {
    return feature.set('class', ee.Number(1)); // Set 'class' as a numeric value
});

var open_space = open_space.map(function(feature) {
    return feature.set('class', ee.Number(2));
});

var greenery = greenery.map(function(feature) {
    return feature.set('class', ee.Number(3));
});

var built_up = built_up.map(function(feature) {
    return feature.set('class', ee.Number(4));
});

// Merge the training data
var training = water.merge(open_space).merge(greenery).merge(built_up);

print(training, 'Training Data 2001');

// Define the label and bands for classification (adjusted for Landsat 7 bands)
var label = 'class';
...
print(trainImage, 'Training Image 2001');
...
Map.addLayer(classified.clip(mysore), {palette: landcoverPalette, min: 1, max: 4}, 'Land Cover Classification 2001');
...
print('Confusion Matrix 2001:', confusionMatrix);
print('Overall Accuracy 2001:', confusionMatrix.accuracy());
print('Kappa Statistic 2001:', confusionMatrix.kappa());
...
print('Class Pixel Count 2001:', classPixelCount);

// Export the classified image to Google Drive
Export.image.toDrive({
    image: classified.clip(mysore),
    description: '2001_Mysuru_RF_Classification', // Ensure this name is unique and valid
    folder: 'GEE',
    region: mysore,
    crs: 'EPSG:32643',
    scale: 30,
...
classPixelCount.get('classification').evaluate(function(counts) {
    ...
    .setOptions({
        title: 'Land Cover Classification 2001',
        slices: {
